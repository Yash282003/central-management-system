...
export default function StudentMess() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Weekly Mess Menu</h1>
        <p className="text-slate-600 mt-1">Meal schedule for the hostel mess</p>
      </div>
      ...
      {sortedMenu.length === 0 ? (
        <Card className="rounded-2xl border-0 shadow-sm">
          <CardContent className="py-20 text-center">
            ...
            <p className="text-slate-500 font-medium">Menu not available yet</p>
            <p className="text-sm text-slate-400 mt-1">
              Check back later — the warden hasn&apos;t uploaded this week&apos;s menu yet.
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50">
                      <th className="text-left px-6 py-4 text-slate-500 font-medium w-36">Day</th>
                      {meals.map((m) => (
                        <th key={m.key} className="text-left px-4 py-4 text-slate-500 font-medium">
                      ...
                      return (
                        <tr
                          ...
                          className={`border-b border-slate-50 last:border-0 transition-colors ${
                            isToday ? "bg-emerald-50/60" : "hover:bg-slate-50/50"
                          }`}
                        >
