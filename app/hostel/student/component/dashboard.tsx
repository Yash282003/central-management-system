...
function GlassStat({ label, value, icon, caption, delay, numeric }: GlassStatProps) {
  const counted = useCounter(numeric ?? 0);
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p
            ...
            style={{
              fontFamily:
                "'Bricolage Grotesque', 'DM Serif Display', serif",
            }}
          >
            {display}
          </p>
          {caption && (
            <p className="mt-1 text-xs text-emerald-900/60">{caption}</p>
          )}
        </div>
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        if (meRes.status === "fulfilled") setStudentInfo(meRes.value ?? null);
          ...
          <div style={{ animation: "fadeSlideUp 0.5s ease both" }}>
            ...
            <h1 className="hostel-display mt-3 text-[2.6rem] leading-[1.05] font-semibold text-emerald-950">
              ...
              <span className="bg-gradient-to-br from-emerald-700 via-green-600 to-teal-600 bg-clip-text text-transparent">
                {studentInfo?.name || "friend"}
              </span>
              <span className="text-emerald-900/40">.</span>
            </h1>
            <p className="mt-2 text-emerald-900/70">
              Welcome home. Here is what is happening at the hostel.
            </p>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <p
                    className="hostel-display text-4xl font-semibold"
                  >
                    {room.roomNo}
                  </p>
                </div>
